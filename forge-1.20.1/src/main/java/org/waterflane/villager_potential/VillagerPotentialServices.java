...
/** Creates the supported versioned service for a logical Forge server. */
public final class VillagerPotentialServices {
    ...
    public static VillagerPotentialService forServer(MinecraftServer server) {
        return new ForgeService(Objects.requireNonNull(server, "server"));
    }

    private static final class ForgeService implements VillagerPotentialService {
        private final MinecraftServer server;

        private ForgeService(MinecraftServer server) {
            this.server = server;
        }
